...
void ProcStat::UpdateCpuStat(TimePoint now, ForceUpdate force) {
  ...
  const std::optional<uint64_t> cpu_ticks = ReadProcessCpuTicks();
  if (!cpu_ticks) {
  ...
  // and when no wall-clock time has elapsed. The diff is taken in native
  // tick space (cumulative ticks never wrap in realistic uptime on
  // Linux/BSD/Windows) and only the small diff is converted to ns, so the
  // old cumulative-ns wraparound after ~9 years (64 cores) cannot corrupt
  // the reading.
  if (time_point_.time_since_epoch().count() != 0 && elapsed_ms > 0) {
    const uint64_t tick_diff = (*cpu_ticks >= prev_cpu_ticks_) ? (*cpu_ticks - prev_cpu_ticks_) : 0;
    const uint64_t diff_ns = ProcessCpuTicksToNs(tick_diff);
    const double elapsed_ns = static_cast<double>(elapsed_ms) * 1'000'000.0;
    cpu_load_cached_ = static_cast<int>(static_cast<double>(diff_ns) / elapsed_ns * 100.0);
  }
  time_point_ = now;
